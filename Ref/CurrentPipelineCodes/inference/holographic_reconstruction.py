...
class HolographicReconstructor:
    ...
    def __init__(
        ...
        padding: int = 16,   # padding for reconstruction (smaller = faster FFT)
        device: Optional[str] = None,
        # Backward compatibility params (lazy init handles dimensions)
        im_x: Optional[int] = None,
        im_y: Optional[int] = None,
        use_fp16: bool = False,  # FP16 requires full pipeline support - keep False for now
    ):
        ...
        # Actual padding used (may differ from self.padding if using power-of-2)
        self._pad_h = None
        self._pad_w = None
    ...
    def _initialize_propagation(self, im_height: int, im_width: int):
        ...
        # Minimum padding is self.padding, but we round up to next power of 2
    ...
    def reconstruct_intensity_stack(self, hologram: torch.Tensor) -> torch.Tensor:
        """
        Alias for reconstruct_intensity() for backward compatibility with inference_service.py.
        ...
        Returns:
            Intensity volume of shape [H, W, num_planes] with values in [0, 1]
        """
        return self.reconstruct_intensity(hologram)
